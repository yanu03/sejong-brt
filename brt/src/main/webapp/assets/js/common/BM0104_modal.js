...
var ACTIONS = axboot.actionExtend(fnObj, {
    ...
    PAGE_CHOICE: function (caller, act, data) {
        ...
        if (list.length > 0) {
        	alert(LANG("ax.admin.BM0104G0.modal.alert"));
            /*if (parent && parent.axboot && parent.axboot.modal) {
                parent.axboot.modal.callback(list[0]);
            }*/
        	axboot.ajax({
                type: "POST",
                url: "/api/v1/BM0104G3U0",
                data: JSON.stringify(list),
                callback: function (res) {
                	console.log(res);
                	console.log("갱신 완료 목록");
                	console.log(res.message);
                	console.log(res.message.size);
                	for(var i=0; i < res.message.size; i++){
                		console.log(res.message.Bm);
                		console.log(res.routNm + "(" + res.routId + ")")
                	}
                }
            });
            return false;
